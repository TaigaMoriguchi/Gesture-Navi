...
(() => {
  ...
  // src/lib/overlay.ts
  var STYLE_ID = "gesture-ops-overlay-style";
  var ARROW_MAP = {
    LEFT: "\u2190",
    RIGHT: "\u2192",
    UP: "\u2191",
  ...
  var GestureOverlay = class {
    ...
    updateCursor(normX, normY) {
      const x2 = clamp(normX, 0, 1) * window.innerWidth;
      const y2 = clamp(normY, 0, 1) * window.innerHeight;
      this.cursor.style.left = `${x2}px`;
      this.cursor.style.top = `${y2}px`;
      this.navGauge.style.left = `${x2}px`;
      this.navGauge.style.top = `${y2}px`;
      return { x: x2, y: y2 };
    }
    flashCursorClick() {
    ...
    showNavCharge(direction, progress) {
  ...
  function sanitizeSettings(input) {
    ...
    normalized.windowMs = clamp2(normalized.windowMs, 300, 1500);
    normalized.minStepDistance = clamp2(normalized.minStepDistance, 1e-3, 0.05);
    normalized.axisLockDistance = clamp2(normalized.axisLockDistance, 0.02, 0.2);
    normalized.axisLockRatio = clamp2(normalized.axisLockRatio, 1, 2.5);
    normalized.minConsistency = clamp2(normalized.minConsistency, 0.35, 0.95);
    normalized.angleToleranceDeg = clamp2(normalized.angleToleranceDeg, 10, 45);
    normalized.minSwipeDistance = clamp2(normalized.minSwipeDistance, 0.08, 0.3);
    normalized.maxOffAxisRatio = clamp2(normalized.maxOffAxisRatio, 0.15, 0.8);
    normalized.horizontalCooldownMs = clamp2(normalized.horizontalCooldownMs, 300, 2500);
    normalized.verticalCooldownMs = clamp2(normalized.verticalCooldownMs, 500, 3e3);
    normalized.cursorSmoothing = clamp2(normalized.cursorSmoothing, 0.05, 0.95);
    normalized.pinchStartRatio = clamp2(normalized.pinchStartRatio, 0.2, 0.7);
    normalized.pinchEndRatio = clamp2(normalized.pinchEndRatio, 0.25, 0.9);
    normalized.pinchCooldownMs = clamp2(normalized.pinchCooldownMs, 100, 2e3);
    normalized.pinchDragScrollFactor = clamp2(normalized.pinchDragScrollFactor, 0.8, 4.5);
    normalized.pinchTapMaxMovePx = clamp2(normalized.pinchTapMaxMovePx, 4, 80);
    normalized.pinchTapMaxDurationMs = clamp2(normalized.pinchTapMaxDurationMs, 80, 600);
  ...
  // src/lib/swipe-detector.ts
  var SwipeDetector = class {
    points = [];
    cooldownUntil = 0;
    lockedAxis = null;
    config;
    constructor(config = DEFAULT_SETTINGS) {
      this.config = config;
    }
    setConfig(config) {
      this.config = config;
    ...
    update(x2, y2, nowMs) {
      ...
      if (last) {
        const stepDistance = Math.hypot(x2 - last.x, y2 - last.y);
        if (stepDistance < this.config.minStepDistance) {
          this.trim(nowMs);
          return "NONE";
        }
      }
      this.points.push({ x: x2, y: y2, t: nowMs });
      this.trim(nowMs);
      ...
      const gesture = axis === "x" ? dx > 0 ? "RIGHT" : "LEFT" : dy > 0 ? "DOWN" : "UP";
      if (!this.isAngleAllowed(dx, dy, gesture)) {
        this.points = [current];
        this.lockedAxis = null;
        return "NONE";
      }
      const cooldownMs = axis === "x" ? this.config.horizontalCooldownMs : this.config.verticalCooldownMs;
      this.cooldownUntil = nowMs + cooldownMs;
      this.points = [current];
      this.lockedAxis = null;
      return gesture;
    }
    resolveAxis(dx, dy, current) {
      if (!this.lockedAxis) {
    ...
    isAngleAllowed(dx, dy, gesture) {
      const angle = Math.atan2(dy, dx) * 180 / Math.PI;
      const targetAngle = gesture === "RIGHT" ? 0 : gesture === "LEFT" ? 180 : gesture === "DOWN" ? 90 : -90;
      const delta = Math.abs(((angle - targetAngle) % 360 + 540) % 360 - 180);
      return delta <= this.config.angleToleranceDeg;
    }
    calculateConsistency(axis) {
      if (this.points.length < 2) {
        return 0;
      }
      let total = 0;
      for (let i2 = 1; i2 < this.points.length; i2 += 1) {
        const prev = this.points[i2 - 1];
        const curr = this.points[i2];
    ...
    trim(nowMs) {
      ...
      this.points = this.points.filter((p2) => p2.t >= cutoff);
      if (this.points.length < 2) {
        this.lockedAxis = null;
      }
    }
  ...
  // src/content.ts
  var NAV_CHARGE_START_PX = 92;
  var NAV_CHARGE_AXIS_RATIO = 1.55;
  var NAV_CHARGE_MAX_VERTICAL_DRIFT_PX = 56;
  var NAV_CHARGE_MIN_PULL_PX = 64;
  var NAV_CHARGE_HOLD_MS = 620;
  var NAV_CHARGE_COOLDOWN_MS = 1150;
  var SUPPRESSED_WASM_WARNING_PATTERNS = [
  ...
  var GestureRuntime = class {
    overlay = new GestureOverlay();
    swipeDetector = new SwipeDetector(DEFAULT_SETTINGS);
    ...
    frameRequestId = null;
    smoothedCursorPoint = null;
    cursorScreenPoint = null;
    pinching = false;
    pinchCooldownUntil = 0;
    pinchStartedAtMs = 0;
    pinchDragDistancePx = 0;
    pinchLastPoint = null;
    pinchStartPoint = null;
    pinchHasNavIntent = false;
    navChargeDirection = null;
    navChargeStartedAtMs = 0;
    navCooldownUntil = 0;
    state = { ...DEFAULT_TAB_STATE };
    async init() {
    ...
    async start() {
      ...
      try {
        ...
        const detail = error instanceof Error ? error.message : typeof error === "string" ? error : "\u4E0D\u660E\u306A\u30A8\u30E9\u30FC";
        this.overlay.setCameraStatus(permissionDenied ? "NOT_GRANTED" : "ERROR");
        this.patchState({
          cameraStatus: permissionDenied ? "NOT_GRANTED" : "ERROR",
          handStatus: "NOT_DETECTED",
          errorMessage: permissionDenied ? "\u30AB\u30E1\u30E9\u304C\u672A\u8A31\u53EF\u3067\u3059\u3002popup\u3092\u958B\u3044\u305F\u72B6\u614B\u3067\u518D\u5EA6ON\u306B\u3057\u3066\u304F\u3060\u3055\u3044\u3002" : `\u8A8D\u8B58\u521D\u671F\u5316\u306B\u5931\u6557\u3057\u307E\u3057\u305F: ${detail}`
        });
    ...
    async ensureLandmarker() {
      ...
      try {
        ...
        this.handLandmarker = await Pc.createFromOptions(wasmFileset, {
          baseOptions: {
            ...baseOptions,
            delegate: "GPU"
          },
    ...
    async resolveWasmFileset(forceReload = false) {
      if (forceReload) {
    ...
    updateVirtualCursor(landmarks) {
      ...
      const normalizedX = this.settings.cursorMirrorX ? 1 - this.smoothedCursorPoint.x : this.smoothedCursorPoint.x;
    ...
    handlePinchGesture(landmarks, nowMs) {
    ...
    finishPinch(nowMs) {
      ...
      const canTapClick = this.pinchDragDistancePx <= this.settings.pinchTapMaxMovePx && duration <= this.settings.pinchTapMaxDurationMs && !this.pinchHasNavIntent;
    ...
    updateNavCharge(nowMs, totalDx, totalDy) {
      ...
      if (!this.navChargeDirection) {
        const canStart = absDx >= NAV_CHARGE_START_PX && absDx >= absDy * NAV_CHARGE_AXIS_RATIO && absDy <= NAV_CHARGE_MAX_VERTICAL_DRIFT_PX;
        if (!canStart) {
          return false;
        }
      ...
      const stillAligned = directionalPull >= NAV_CHARGE_MIN_PULL_PX && Math.abs(totalDx) >= Math.abs(totalDy) * NAV_CHARGE_AXIS_RATIO && Math.abs(totalDy) <= NAV_CHARGE_MAX_VERTICAL_DRIFT_PX;
      if (!stillAligned) {
        this.clearNavCharge();
        return false;
      }
      const progress = clamp3((nowMs - this.navChargeStartedAtMs) / NAV_CHARGE_HOLD_MS, 0, 1);
    ...
    commitNavCharge(direction, nowMs) {
      if (direction === "BACK") {
    ...
    resetCursorTransientState() {
      this.overlay.setCursorVisible(false);
      this.smoothedCursorPoint = null;
      this.cursorScreenPoint = null;
      this.resetPinchState();
    }
    resetHandTransientState() {
    ...
    async stop() {
      ...
      this.resetHandTransientState();
      await this.stopCameraOnly();
      const cameraStatus = this.hasCameraPermission ? "GRANTED" : "NOT_GRANTED";
  ...
  var runtime = new GestureRuntime();
  void runtime.init();
  function clamp3(value, min, max) {
    return Math.min(max, Math.max(min, value));
  }
  function shouldSuppressWasmWarning(args) {
